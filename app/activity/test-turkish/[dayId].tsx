...
export default function TestTurkishActivityScreen() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const quizQuestions: QuizQuestion[] = dayWords.map(w => {
                const correctAnswer = w.turkish;

                // Get distractors from OTHER words in the same day
                const others = dayWords.filter(other => other.id !== w.id);
                let otherAnswers = Array.from(new Set(others.map(o => o.turkish)));
    ...
    const handleComplete = async (percentage: number, wordResults: Record<string, boolean>) => {
        await recordActivityResults(dayId, 'testTurkish', wordResults, percentage);
    };
